...
/* USER CODE BEGIN PV */
uint8_t dir = 1, en = 0; float vel = 1;
/* USER CODE END PV */
...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
StepperMotor motorX(1, true, 2.0, 32);
StepperMotor motorY(1, true, 2.0, 32);
...
int main(void)
  ...
  /* USER CODE BEGIN 2 */
	...
	motorX.SetPorts(M1_DIR_GPIO_Port, M1_DIR_Pin, M1_EN_GPIO_Port, M1_EN_Pin);
	motorX.SetPWM(&htim17, TIM_CHANNEL_1);	
	
	motorY.SetPorts(M3_DIR_GPIO_Port, M3_DIR_Pin, M3_EN_GPIO_Port, M3_EN_Pin);
	motorY.SetPWM(&htim3, TIM_CHANNEL_1);

	motorZ.SetPorts(M2_DIR_GPIO_Port, M2_DIR_Pin, M2_EN_GPIO_Port, M2_EN_Pin);
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOC, M2_DIR_Pin|M2_EN_Pin|Orange_Led_Pin|Green_Led_Pin, GPIO_PIN_RESET);
